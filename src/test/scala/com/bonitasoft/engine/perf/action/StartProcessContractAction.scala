...
class StartProcessContractAction(coreComponents: CoreComponents,
                                 bonitaComponents: BonitaComponents,
                                 val statsEngine: StatsEngine,
                                 val next: Action,
                                 val processName: String,
                                 val processVersion: String,
                                 contract: util.Map[String,Serializable]) extends ExitableAction with NameGen with LoggedInAction {
  ...
  override def execute(session: Session): Unit = {
    ...
    val instance = apiClient.getProcessAPI.startProcessWithInputs(processDefinitionId, contract)
